...
class Globalvaluectr extends BaseController
{
    
    private $globalvaluemtr = null;
    
    public function __construct()
    {
        parent::__construct();
        
        $this->globalvaluemtr = model('config/globalvaluemtr', true, $this->db);
        
        $this->global_data["tb_columns"] = $this->globalvaluemtr->tb_columns;
        $this->global_data["columns"] = $this->globalvaluemtr->tb_columns["columns"];
        $this->global_data["class"] = "config/globalvaluectr";
        $this->global_data["view_path"] = "config/globalvalue/";
        $this->global_data["lang_path"] = "config/globalvalue_lang";
    }
    ...
    public function iud_index(){
        ...
        if(count($ajax_data_ar)> 0 )
            ...
            elseif(($flag_ctr == "u" || $flag_ctr == "d") && $this->check_array_key($this->globalvaluemtr->tb_columns["primary_key"], $ajax_data_ar)):
            
            $search_par = array();
            if(is_array($this->globalvaluemtr->tb_columns["primary_key"])):
            foreach ($this->globalvaluemtr->tb_columns["primary_key"] as $m_pk):
            $search_par[$m_pk] = $ajax_data_ar[$m_pk];
            endforeach;
            else:
            $search_par = array($this->globalvaluemtr->tb_columns["primary_key"]=>$ajax_data_ar[$this->globalvaluemtr->tb_columns["primary_key"]]);
            endif;
            
            //$this->index_data["result_q"] = $this->globalvaluemtr->baseselectQuery($search_par);
            $builder = $this->db->table($this->globalvaluemtr->table);
    ...
    private function findmasterq_for_view(){
        ...
        $column_ar = $this->globalvaluemtr->tb_columns["columns"];
    ...
    public function select(){
        ...
        //$sql = "select um.*,(select name from globalvaluemtr where um.globalvalue_id = globalvaluemtr.globalvalue_id ) as globalvalue_name,(select name from globalvaluemtr where um.globalvalue_id = globalvaluemtr.globalvalue_id ) as globalvalue_name from globalvalueglobalvalue um";
        //$data["data"] = $this->db->query($sql,array())->getResult('array');
        $data["data"] = $this->globalvaluemtr->findAll();
        echo json_encode($data);
