...
class Carctr extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->carmtr = model('car/carmtr', true, $this->db);
        
        $this->global_data["tb_columns"] = $this->carmtr->tb_columns;
        $this->global_data["columns"] = $this->carmtr->tb_columns["columns"];
        $this->global_data["class"] = "car/carctr";
        $this->global_data["view_path"] = "car/carmtr/";
        $this->global_data["lang_path"] = "car/carmtr_lang";
    }
	...
	public function iud_index(){
	    ...
	    if(count($ajax_data_ar)> 0 )
	        ...
	        elseif(($flag_ctr == "u" || $flag_ctr == "d") && $this->check_array_key($this->carmtr->tb_columns["primary_key"], $ajax_data_ar)):
	        
    	        $search_par = array();
    	        if(is_array($this->carmtr->tb_columns["primary_key"])):
    	           foreach ($this->carmtr->tb_columns["primary_key"] as $m_pk):
    	               $search_par[$m_pk] = $ajax_data_ar[$m_pk];
    	           endforeach;
    	        else:
    	           $search_par = array($this->carmtr->tb_columns["primary_key"]=>$ajax_data_ar[$this->carmtr->tb_columns["primary_key"]]);
    	        endif;

    	        //$this->index_data["result_q"] = $this->carmtr->baseselectQuery($search_par);
    	        $builder = $this->db->table($this->carmtr->table);
	...
	private function findmasterq_for_view(){
	    ...
	    if(is_array($this->carmtr->tb_columns["primary_key"])):
    	    ...
    	    $column_ar = $this->carmtr->tb_columns["columns"];
