...
class Gpseventctr extends BaseController
{
    
    private $gpsevent = null;
    
    public function __construct()
    {
        parent::__construct();
        
        $this->gpsevent = model('gps/gpsevent', true, $this->db);
        
        $this->global_data["tb_columns"] = $this->gpsevent->tb_columns;
        $this->global_data["columns"] = $this->gpsevent->tb_columns["columns"];
        $this->global_data["class"] = "gps/gpseventctr";
        $this->global_data["view_path"] = "gps/gpsevent/";
        $this->global_data["lang_path"] = "gps/gpsevent_lang";
    }
	...
	public function iud_index(){
	    ...
	    if(count($ajax_data_ar)> 0 )
	        ...
	        elseif(($flag_ctr == "u" || $flag_ctr == "d") && $this->check_array_key($this->gpsevent->tb_columns["primary_key"], $ajax_data_ar)):
	        
    	        $search_par = array();
    	        if(is_array($this->gpsevent->tb_columns["primary_key"])):
        	        foreach ($this->gpsevent->tb_columns["primary_key"] as $m_pk):
        	           $search_par[$m_pk] = $ajax_data_ar[$m_pk];
        	        endforeach;
        	    else:
        	       $search_par = array($this->gpsevent->tb_columns["primary_key"]=>$ajax_data_ar[$this->gpsevent->tb_columns["primary_key"]]);
                endif;
                
    	        //$this->index_data["result_q"] = $this->gpsevent->baseselectQuery($search_par);
    	        $builder = $this->db->table($this->gpsevent->table);
	...
	private function findmasterq_for_view(){
	    ...
	    if(is_array($this->gpsevent->tb_columns["primary_key"])):
    	    ...
    	    $column_ar = $this->gpsevent->tb_columns["columns"];
	...
	public function select(){
	    ...
	    $data["data"] = $this->gpsevent->findAll();
	    echo json_encode($data);
